...
y_table = np.array(
    ...
    dtype=np.float32).T
y_table = paddle.to_tensor(y_table)
...
c_table = paddle.to_tensor(c_table)
...
class RGB2YCbCrJpeg(nn.Layer):
    ...
    def __init__(self):
        ...
        matrix = np.array([[0.299, 0.587, 0.114], [-0.168736, -0.331264, 0.5], [0.5, -0.418688, -0.081312]],
                          dtype=np.float32).T
        paddle.disable_static()
        self.matrix = paddle.to_tensor(matrix)#paddle.create_parameter([3, 3], dtype='float32', default_initializer=matrix)
        # ww = np.array([0., 128., 128.], dtype=np.float32)
        self.shift = paddle.to_tensor(np.array([0., 128., 128.], dtype=np.float32))#paddle.create_parameter([1, 3], dtype='float32', default_initializer=ww)
...
class DCT8x8(nn.Layer):
    ...
    def __init__(self):
        ...
        scale = paddle.to_tensor(np.outer(alpha, alpha) * 0.25)
...
class iDCT8x8(nn.Layer):
    ...
    def __init__(self):
        ...
        alpha = np.array([1. / np.sqrt(2)] + [1] * 7)
        alpha = paddle.to_tensor(np.outer(alpha, alpha))
...
class YCbCr2RGBJpeg(nn.Layer):
    ...
    def __init__(self):
        ...
        self.shift = paddle.to_tensor([0, -128., -128.])#paddle.create_parameter(tensor.shape, 'float32', default_initializer=paddle.nn.initializer.Assign(tensor))
        matrix = paddle.to_tensor(matrix)
        self.matrix = paddle.to_tensor(matrix)#paddle.create_parameter(matrix, 'float32', default_initializer=paddle.nn.initializer.Assign(matrix))
...
if __name__ == '__main__':
    ...
    img_gt = paddle.stack([img_gt, img_gt]).cuda()
