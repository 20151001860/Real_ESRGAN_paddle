...
def filter2D(img, kernel):
    ...
    if kernel.size(0) == 1:
        ...
        return F.conv2d(img.astype(paddle.float32), kernel.astype(paddle.float32), padding=0).view(b, c, h, w)
    else:
        img = img.view(1, b * c, ph, pw)
        kernel = kernel.view(b, 1, k, k).repeat(1, c, 1, 1).view(b * c, 1, k, k)
        return F.conv2d(img.astype(paddle.float32), kernel.astype(paddle.float32), groups=b * c).view(b, c, h, w)
...
class USMSharp(paddle.nn.Layer):
    ...
    def __init__(self, radius=50, sigma=0):
        ...
        kernel = paddle.to_tensor(np.dot(kernel, kernel.transpose()).astype(np.float32)).unsqueeze_(0)
